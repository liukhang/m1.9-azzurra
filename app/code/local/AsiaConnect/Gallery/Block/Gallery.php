...
class AsiaConnect_Gallery_Block_Gallery extends Mage_Core_Block_Template
{
    ...
    {
    	//Zend_debug::dump($this->getRequest()->getParam('id'));die;
    	...
    	//get tags meta for gallery
    	$cate = Mage::getModel('gallery/album')->load($this->getRequest()->getParam('id'),'album_id');
    	if ($this->getLayout()->getBlock('head')) {
		 		$head = $this->getLayout()->getBlock('head');
				if($cate->getTitle()){
		        $head->setTitle( $cate->getTitle() );
				}else{
					$head->setTitle('Realizzazioni');
				}
		        $head->setDescription($cate->getMetaDescription());
		        $head->setKeywords($cate->getMetaKeyword());
		}
